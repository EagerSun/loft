...
class Optimizer(BaseMethod):
    ...
    def __init__(self, config: object = None):
        self.adamW = partial(paddle.optimizer.AdamW, beta1=config.betas1, beta2=config.betas2, epsilon=config.eps, weight_decay=config.weight_decay)
        return
    ...
    def __call__(self, model_params: object = None, key: str = None):
        ...
        if key is None:
            for _key in _d_optimizer:
                _d_optimizer[_key] = _d_optimizer[_key](parameters=model_params)
            return _d_optimizer
        else:
            _d_optimizer[key] = _d_optimizer[key](parameters=model_params)
...
# Preprecess for training/validating is defined here!
...
def _preprocesses():
    ...
    def txt(string: str = None):
        ...
        return {"input_ids": paddle.to_tensor(ids), "type_ids": paddle.to_tensor(type_ids)}
